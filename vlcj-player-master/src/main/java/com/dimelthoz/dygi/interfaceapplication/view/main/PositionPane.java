...
final class PositionPane extends JPanel {
        ...
        positionSlider.addChangeListener(new ChangeListener() {
            ...
            public void stateChanged(ChangeEvent e) {
                if (!positionChanging.get()) {
                    ...
                    Application.application().mediaPlayer().controls().setPosition(source.getValue() / 1000.0f);
                }
            }
        });
    ...
    private void refresh() {
        ...
        if (!sliderChanging.get()) {
            int value = (int) (Application.application().mediaPlayer().status().position() * 1000.0f);
