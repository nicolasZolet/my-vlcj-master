...
import com.dimelthoz.dygi.interfaceapplication.view.action.mediaplayer.SubtitleTrackAction;
import uk.co.caprica.vlcj.player.base.TrackDescription;

final class SubtitleTrackMenu extends TrackMenu {

    SubtitleTrackMenu() {
        super(Resource.resource("menu.subtitle.item.track"));
    }
    ...
    protected List<TrackDescription> onGetTrackDescriptions() {
        return Application.application().mediaPlayer().subpictures().trackDescriptions();
    }

    @Override
    protected int onGetSelectedTrack() {
        return Application.application().mediaPlayer().subpictures().track();
    }
}
